export function RecoveryCodesPage({
  ...
  return <div className="platform-panel platform-panel--narrow">
    ...
    <Checkbox id="platform-recovery-acknowledged" label="我已将恢复码保存在安全位置" checked={acknowledged}
      onChange={(event) => onAcknowledgedChange(event.currentTarget.checked)} />
    <Button className="platform-wide-action" disabled={!acknowledged} onClick={onContinue}>继续登录</Button>
  </div>;
}
