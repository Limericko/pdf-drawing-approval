...
export class StorageObjectService {
  ...
  async create(input: { readonly body: Readable; readonly mediaType: string }): Promise<StorageObject> {
    ...
    const written = await this.dependencies.storage.write(staged.objectKey, input.body, input.mediaType);
...
export async function requestStorageObjectDeletion(options: DeleteStorageObjectOptions) {
  return options.transactionRunner(async (executor) => {
    await options.removeReferences(executor);
    const object = await options.createRepository(executor).markDeletePending(
      options.storageObjectId,
      new Date(options.requestedAt.getTime())
    );
    await options.publisher.publish(executor, createCleanupIntent(object, "delete_pending"));
    return object;
  });
}
...
function assertMediaType(mediaType: string) {
  if (typeof mediaType !== "string" || !mediaType.trim() || mediaType.length > 255) {
