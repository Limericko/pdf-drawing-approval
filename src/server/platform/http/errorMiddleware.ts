...
const DOMAIN_PROBLEMS: Readonly<Record<string, ProblemMapping>> = Object.freeze({
  ...
  PASSWORD_CHANGE_REQUIRED: { status: 403, title: "Initial password change required" },
  ACCOUNT_INPUT_INVALID: { status: 400, title: "Invalid account update" },
  ACCOUNT_INVALID_CREDENTIALS: { status: 401, title: "Authentication required" },
  ACCOUNT_PASSWORD_REQUIRED: { status: 422, title: "New password required" },
  ACCOUNT_CONFLICT: { status: 409, title: "Account identifier already exists" },
  ACCOUNT_DEPENDENCY_UNAVAILABLE: { status: 503, title: "Account service unavailable" },
