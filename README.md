...
只有一台云服务器时，优先使用单机完整版。它将应用、Worker、PostgreSQL 和 MinIO 作为一套产品管理，并提供统一的安装、升级、备份、恢复和健康检查入口；Web 默认仅绑定 `127.0.0.1:18080`，由宝塔/Nginx 等现有入口自行配置 HTTPS 反向代理。正式高可用环境继续使用云厂商无关的生产 Compose。
