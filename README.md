...
```bash
docker pull ghcr.io/limericko/pdf-drawing-approval@sha256:347aeafd1e9fcf54baab871efe8bee0d40191fad433141350ded2249934c47bb
```

生产环境使用不可变 digest：

```text
PDF_APPROVAL_IMAGE=ghcr.io/limericko/pdf-drawing-approval@sha256:347aeafd1e9fcf54baab871efe8bee0d40191fad433141350ded2249934c47bb
```
...
只有一台云服务器时，优先使用单机完整版。它将应用、Worker、PostgreSQL 和 MinIO 作为一套产品管理，并提供统一的安装、升级、备份、恢复和健康检查入口；安装时只需填写域名和本地反代端口，系统自动创建 `admin` 管理员并强制首次改密，邮箱与 SMTP 均在登录后的管理台配置。Web 默认仅绑定 `127.0.0.1:18080`，由宝塔/Nginx 等现有入口自行配置 HTTPS 反向代理。正式高可用环境继续使用云厂商无关的生产 Compose。
