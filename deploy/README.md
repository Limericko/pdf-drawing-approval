...
当前 `main` 构建的不可变镜像为 `ghcr.io/limericko/pdf-drawing-approval@sha256:acb300ef17b1ea0a70b1249ae4db4e07573405229bce9e9d3a2b461cd97419b9`。部署时使用完整 digest，不使用可变标签 `0.9.2-refactor`。2026-07-15 已通过匿名 GHCR token 验证：manifest 返回 HTTP 200，且 `Docker-Content-Digest` 与上述 digest 一致。
