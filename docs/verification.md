...
- `npm test -- --run src/client src/server/repositories src/server/routes`：83 个文件，`614/614` 通过。
- `npm run e2e:typecheck`：通过。
...
- 正式问题浏览器闭环覆盖主管创建 high 问题、设计师开始处理并提交复核、主管独立关闭，以及 high 问题阻断审批。
- legacy Playwright：28 项，`24 passed / 4 skipped`；跳过项均为按产品策略不适用于手机或桌面专属契约的用例。
- UI Gallery：1440、1280、1024、768、390 五视口 `5/5` 通过。
- Platform Playwright：desktop identity `1/1`、desktop project/session `2/2`、mobile identity `1/1`，合计 `4/4` 通过。
