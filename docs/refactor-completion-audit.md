...
| Phase 6 迁移与部署 | 仓库实现和本地容器验收已完成 | 通用 Dockerfile/Compose、单机 Caddy/PostgreSQL/MinIO 运行包、只读秘密、迁移五阶段、幂等 delta/verify、非 root 容器和 readiness | 真实生产部署、正式迁移和切换未执行 |
...
| 单机完整版 | Compose 展开、Shell 语法、PostgreSQL/MinIO 初始化、10 项 migration、Web 健康检查和 Worker 启动通过；只有 Caddy 发布 80/443 |
