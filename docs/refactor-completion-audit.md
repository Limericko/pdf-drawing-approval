...
- 审计日期：2026-07-14
- 审计分支：`codex/phase-6-hk-cloud-deployment`
...
当前仓库可以提交并构建 OCI 镜像，但不能声称已经完成正式生产切换。生产完成仍依赖真实域名、正式账号邮箱、目标容器平台、PostgreSQL、S3、SMTP、WebDAV、密钥管理、维护窗口以及故障/恢复证据。
...
4. Registry 推送、SBOM/依赖扫描、镜像签名和以不可变 digest 部署的证据。
...
仓库内重构任务已达到提交条件。`git diff --check`、Compose 展开、最终镜像构建以及 Web、Worker、migration 容器冒烟已经通过。上述本地镜像 ID 只证明本机构建内容；正式发布仍须推送 Registry、扫描、签名，并以 Registry 返回的不可变 digest 部署。
