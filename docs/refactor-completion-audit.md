...
- 审计日期：2026-07-14；公开仓库、主分支整合与公开镜像复核：2026-07-15
...
Phase 0–5 的计划内代码、自动化测试和浏览器闭环已完成。Phase 6 的仓库内交付物已经完成：生产配置与文件密钥注入、单一非 root 镜像、通用 Compose、Web/Worker/migration/bootstrap/legacy-migration 入口、SQLite 与文件迁移工具、校验和增量迁移链路，以及面向单台 Linux 云服务器的一键安装与统一运维包。

GitHub 代码仓库已经公开，Phase 0–6 已整合到 `main`。GHCR 镜像已由 GitHub Actions 推送并附带 SBOM/provenance，容器包也已切换为公开可见性并通过匿名 manifest 验证。仓库与镜像可以用于部署准备，但不能声称已经完成正式生产切换；生产完成仍依赖真实域名、正式账号邮箱、目标容器平台、PostgreSQL、S3、SMTP、WebDAV、密钥管理、维护窗口以及故障/恢复证据。
...
| Phase 6 迁移与部署 | 仓库实现和本地容器验收已完成 | 通用 Dockerfile/Compose、单机 PostgreSQL/MinIO 与外部反代运行包、只读秘密、迁移五阶段、幂等 delta/verify、非 root 容器和 readiness | 真实生产部署、正式迁移和切换未执行 |
...
| 门禁 | 结果 |
| --- | --- |
| `npm test -- --run` | 209 个文件，1491 通过，3 项按平台策略跳过 |
...
| 单机完整版 | Compose 展开、Shell 语法、PostgreSQL/MinIO 初始化、11 项 migration、默认管理员幂等创建、首次强制改密、无 SMTP 启动、Web 健康检查和 Worker 启动通过；Web 仅绑定可配置的宿主机回环端口，证书和反代由外部入口负责 |
| 最终 OCI 镜像 | GitHub Actions 从 `main` 干净构建并推送 `pdf-approval:0.9.2-refactor` |
| GHCR 发布 | `main` 镜像 digest 为 `sha256:6fe7106cb6422d4d9e8b346a249e9551178134ce7046d67b8ef2ae8eeb582eda`，并生成 SBOM/provenance；匿名 manifest 请求返回 HTTP 200，`Docker-Content-Digest` 一致 |
